...
class UtilsData: NSObject {
    ...
    var modelForms: [ModelForms] = []

    var currentIndex = 0
    var fm = FileManager.default
    var fresult: Bool = false
    var subUrlForm: URL?
    var mainUrlForm: URL? = Bundle.main.url(forResource: "forms", withExtension: "json")
    var subUrlPlan: URL?
    ...
    func decodeDataForm(pathName: URL) {
        do {
            ...
            modelForms = try decoder.decode([ModelForms].self, from: jsonData)
